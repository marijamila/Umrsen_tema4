...
static int16_t c0, c1, c20, c30, c01, c11, c21;
...
//	cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, data, 2, 0, true);
	...
	// read COEF
	for (uint8_t i = 0; i < COEF_SIZE; i++) {
		cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, &coef_addr[i], 1, 0, false);
		cyhal_i2c_master_read(&i2c_master_obj, I2C_SLAVE_ADDRESS, &coef[i], 1, 0, true);
	}
	...
	if (CY_RSLT_SUCCESS != cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, data, 2, 0, true)) {
		CY_ASSERT(0);  // nije CY_RSLT_ADDRESS???
	}
}
...
void DPS310_measure(float *temperature, float *pressure) {
	...
	// read temperature
	if (CY_RSLT_SUCCESS == cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, meas, 2, 0, true)) {
		for (int i = 0; i < 3; i++) {
			cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, &tmp_b[i], 1, 0, false);
			cyhal_i2c_master_read(&i2c_master_obj, I2C_SLAVE_ADDRESS, &tmp[i], 1, 0, true);
		}
	...
	if (CY_RSLT_SUCCESS == cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, meas, 2, 0, true)) {
		...
		for (int i = 0; i < 3; i++) {
			cyhal_i2c_master_write(&i2c_master_obj, I2C_SLAVE_ADDRESS, &prs_b[i], 1, 0, false);
			cyhal_i2c_master_read(&i2c_master_obj, I2C_SLAVE_ADDRESS, &prs[i], 1, 0, true);
		}
